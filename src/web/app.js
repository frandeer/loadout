...
function promptFor(preset, it) {
  ...
  const identityRule = "Translate the asset's name and description into a visual metaphor. Do not copy the tool name into the image. Show objects, actions, state changes, before/after cues, arrows, plugs, shields, windows, servers, charts, or other visual symbols that explain the purpose. Make the purpose recognizable at a glance with ONE single clear, flat, literal icon-like scene. Pick the single best-fitting visual metaphor. Keep it simple: one focal subject, clean light background, generous empty space, flat minimal style. Text rule: no title, no tool name, no speech bubble, no object labels, no screen text, no command text, no code text, no gauge letters, no numbers, no pseudo-text. Allow exactly one small bottom caption only: 2-5 plain words, core idea only, no punctuation.";
